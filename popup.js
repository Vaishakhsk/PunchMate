document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('manualClockIn').addEventListener('click', function() {
    ...
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
      if (tabs.length === 0) {
        ...
        chrome.tabs.create({url: 'https://newstreet.keka.com/#/home/dashboard'}, function(tab) {
        ...
        if (activeTab.url.includes('newstreet.keka.com')) {
          ...
          chrome.tabs.update(activeTab.id, {url: 'https://newstreet.keka.com/#/home/dashboard'}, function(tab) {
            // Wait for page to load before sending message
            setTimeout(() => {
              sendClockInMessage(tab.id, actionStatus);
            }, 5000);
  ...
  document.getElementById('manualClockOut').addEventListener('click', function() {
    const actionStatus = document.getElementById('actionStatus');
...
function sendClockInMessage(tabId, actionStatus) {
  ...
  chrome.scripting.executeScript({
    ...
    chrome.tabs.sendMessage(tabId, {action: 'clockIn'}, function(response) {
      if (chrome.runtime.lastError) {
        actionStatus.textContent = 'Error: ' + chrome.runtime.lastError.message;
        actionStatus.style.backgroundColor = '#f8d7da';
...
function sendClockOutMessage(tabId, actionStatus) {
  ...
  chrome.scripting.executeScript({
    ...
    // Now send the message to the content script
    chrome.tabs.sendMessage(tabId, {action: 'clockOut'}, function(response) {
      if (chrome.runtime.lastError) {
        actionStatus.textContent = 'Error: ' + chrome.runtime.lastError.message;
        actionStatus.style.backgroundColor = '#f8d7da';
      } else if (response && response.success) {
...
function detectClockStatus() {
  try {
    ...
    const clockInButton = document.querySelector('button.btn.btn-x-sm.mx-4.btn-white');
    ...
    const clockOutButton = document.querySelector('button.btn.btn-danger.btn-x-sm');
