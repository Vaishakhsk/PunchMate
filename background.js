    ...
    if (tabs.length > 0) {
      ...
      chrome.tabs.update(
        ...
        function (tab) {
          ...
          console.log(
            `Tab found, waiting for page to load before executing ${action} action`
          );
          setTimeout(() => executeClockAction(tab.id, action, today), 8000);
        }
      );
    } else {
      // Open Keka in a new tab
      console.log(`Opening new tab for ${action} action`);
      chrome.tabs.create(
        {
          url: "https://newstreet.keka.com/#/home/dashboard",
        },
        function (tab) {
          // Wait for page to load, then inject the content script
          console.log(
            `New tab opened, waiting for page to load before executing ${action} action`
          );
          setTimeout(() => executeClockAction(tab.id, action, today), 12000);
        }
      );
    }
  });
  ...
  try {
    ...
    setTimeout(() => {
      ...
      if (button) {
        ...
        button.click();
        console.log(
          `Clicked the ${action === "in" ? "Clock In" : "Clock Out"} button`
        );
        return true;
      } else {
